...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="sticky top-0 z-30 flex h-16 w-full items-center justify-between border-b border-slate-200 bg-white/95 px-3 sm:px-4 md:px-6 backdrop-blur-md dark:border-slate-800 dark:bg-slate-900/95">
      {/* Left: Mobile Toggle & Page Title */}
      <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1 mr-2">
        <button
          onClick={onOpenMobileMenu}
          className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-slate-200 text-slate-700 hover:bg-slate-100 md:hidden dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800"
        ...
        <div className="flex flex-col min-w-0">
          <h2 className="text-sm sm:text-base md:text-lg font-bold text-slate-900 dark:text-white leading-tight truncate">
            {info.title}
          </h2>
      ...
      <div className="flex items-center gap-1.5 sm:gap-2 md:gap-3 shrink-0">
        ...
        <div
          className={`hidden lg:flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold border ${
            dbStatus === 'connected'
        ...
        >
          ...
          <span className="hidden xl:inline">Banco Central:</span>
        ...
        {/* User profile pill */}
        {currentUser && (
          <div className="hidden xl:flex items-center gap-1.5 rounded-full bg-slate-100 dark:bg-slate-800 px-2.5 py-1 text-xs text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700">
        ...
        {isInstallable && (
          <button
            onClick={install}
            className="hidden xl:inline-flex items-center gap-1.5 rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-1.5 text-xs font-semibold text-indigo-700 hover:bg-indigo-100 dark:border-indigo-800 dark:bg-indigo-950/40 dark:text-indigo-300 transition shrink-0"
          >
